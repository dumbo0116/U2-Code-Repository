...
root = tk.Tk()
root.title("William Shi's Music Recommendation App")
...
def reset():
    ...
    # Store square IDs and square data
    global squares, square_data, questions
    questions = [
        "How stressed are you right now, on a scale of 1-5?",
        "How happy or content are you with life right now, on a scale of 1-5?",
        "How concerned or worried are you right now, on a scale of 1-5?",
...
def handle_click(event):
    ...
    global squares, square_data, questions, responses
    ...
    # INPUT VALIDATION LOOP ADDED HERE
    while True:
        user_input = simpledialog.askstring("Evaluation Question", question)
        ...
        if user_input == "":
            messagebox.showerror("Invalid Input", "Please enter a number from 1 to 5.")
            continue

        if not user_input.isdigit():
            messagebox.showerror("Invalid Input", "Input must be a whole number from 1 to 5.")
            continue

        value = int(user_input)

        if value < 1 or value > 5:
            messagebox.showerror("Invalid Input", "Number must be between 1 and 5.")
            continue
    ...
    if ("" not in responses):
        ...
        if messagebox.askyesno("Result", "Would you like to retake the evaluation survey?"):
            reset()
        else:
            messagebox.showinfo("Result", "Thank you and have a good day!")
